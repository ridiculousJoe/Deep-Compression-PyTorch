...
def huffman_encode_model(model, directory='encodings/'):
    ...
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            #Encode
            t0, d0 = huffman_encode(bias, name+f'_{form}_bias',directory) 
    ...
    print(original_total, compressed_total)
...
def huffman_decode_model(model, directory='encodings/'):
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            dev = param.device
            # decode bias
            bias = huffman_decode(directory, name+f'_{form}_bias', dtype = 'float32')
            # insert bias into model
